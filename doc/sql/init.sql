...
(
    ...
    `total_amount`     decimal(15, 2) DEFAULT 0.00 not null comment '总现金资产',
    `monthly_expenses` decimal(15, 2) DEFAULT 0.00 not null comment '每月支出',
    `monthly_income`   decimal(15, 2) DEFAULT 0.00 not null comment '每月收入',
...
(
    ...
    `account_id`       varchar(64) NOT NULL comment '账户ID',
    `ts_code`          VARCHAR(16) NOT NULL COMMENT '股票的TS代码',
    `shares`           INT         NOT NULL DEFAULT 0 COMMENT '持仓数量',
    `plan_type`        INT         NOT NULL DEFAULT 1 COMMENT '计划类型：1：leftBuy 2: rightBuy 3：aiBuy',
    `plan_amount`      DECIMAL(18, 2) COMMENT '计划买入金额',
    `used_amount`      DECIMAL(18, 2) COMMENT '已用资金',
    `available_amount` DECIMAL(18, 2) comment '可用资金',
    `pct_chg`          DECIMAL(18, 2) COMMENT '涨跌幅',
    `buy_avg_price`    DECIMAL(18, 2) COMMENT '买入均价',
    `current_price`    DECIMAL(18, 2) COMMENT '当前价格',
    `current_pe`       DECIMAL(18, 2) COMMENT '当前pe',
    `total_loss`       DECIMAL(18, 2) COMMENT '总亏损',
    `total_buy_amount` DECIMAL(18, 2) COMMENT '买入总金额',
    `current_total_mv` DECIMAL(18, 2) COMMENT '总市值',
    `change`           DECIMAL(18, 2) COMMENT '涨跌额',
